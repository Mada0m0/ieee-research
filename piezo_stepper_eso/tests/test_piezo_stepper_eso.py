...
class TestPiezoStepperESO(unittest.TestCase):
    ...
    def test_eso_convergence(self):
        ...
        for _ in range(500):
            ...
            # In the open loop case, u remains 0
            u_prev = 0.0

        # The perturbation should converge
        z3_est = eso.get_disturbance()
        # The internal disturbance term of plant, z3 is estimated to be opposite or similar to it.
        ...
        # Here it is sufficient to verify that z3 eventually converges, just check that it does not diverge and has a clear value.
    ...
    def test_step_response(self):
        ...
        # Verify there is no severe overshoot (allow a little overshoot due to non-linearities such as hysteresis)
        max_overshoot = np.max(y_seq) - 1.0
        self.assertLess(max_overshoot, 0.1, "Overshoot is too large")

    def test_sine_tracking(self):
        """Sine Tracking Test: Verify In-Bandwidth Tracking Accuracy"""
        n_steps = 2000
        ...
        # Verify the mean absolute error for subsequent periods
    ...
    def test_disturbance_rejection(self):
        ...
        # After the perturbation is applied and adjusted over a period of time, it should re-stabilize
