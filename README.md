...
- **`jules_jobs/`** – 预构建的 Jules 任务 JSON 文件
- **`implementations/`** – Jules 生成的最终代码（按类别组织）
...
├── implementations/                    # Jules 生成的代码
│   ├── hysteresis/                     # 迟滞模型
│   │   ├── generalized_bouc_wen.py     # GBW 模型 + PSO 辨识 (PR #2)
│   │   ├── rmso_bw_model.py            # RMSO-BW 模型 (PR #10)
│   │   ├── rmso_optimizer.py           # RMSO 优化器
│   │   └── test_generalized_bouc_wen.py
│   ├── controls/                       # 控制算法
│   │   ├── piezo_stepper_eso.py        # 压电步进电机 + ADRC (PR #3)
│   │   ├── piezo_walking_motor.py      # 压电行走电机 (PR #4)
│   │   ├── pmn_pt_shear_actuator.py    # PMN-PT 剪切执行器 (PR #5)
│   │   ├── fuzzy_nn_controller.py      # 模糊神经网络控制器 (PR #10)
│   │   ├── rmso_bw_compensator.py      # RMSO-BW 逆前馈 + 模糊NN补偿器 (PR #10)
│   │   └── *.png                       # 可视化输出 (电机位移/步态)
│   └── voltage_waveform/               # 电压波形生成与优化 (PR #6)
│       ├── generators.py               # 梯形波、锯齿波生成器
│       ├── optimizer.py                # 遗传算法优化器
│       └── tests/
├── jules_jobs/                         # Jules 任务定义
│   ├── generalized_bouc_wen.json
│   ├── piezo_stepper_eso.json
│   ├── piezo_walking_motor.json
│   ├── pmn_pt_shear_actuator.json
│   └── rmso_bw_fuzzy_nn.json          # 新 (PR #10)
├── references/                         # 论文引用与背景文档
│   ├── piezo_leg_waveform_design.md
│   └── liu2020_rmso_bw.md             # RMSO-BW 论文摘要
├── research/templates/                 # 研究领域模板 (YAML)
├── tests/                              # 单元测试 (pytest)
│   └── test_rmso_bw.py                # RMSO-BW 模型测试
├── examples/                           # Jupyter 示例
│   └── example_rmso_bw_control.ipynb  # RMSO-BW + 模糊NN 闭环跟踪示例
├── hub.py                              # 中央调度器
...
合并后，生成的代码即纳入 `implementations/` 主分支。
...
执行步骤：
1. **代码风格**：`pycodestyle implementations/ tests/ examples/`
2. **类型检查**：`mypy --ignore-missing-imports implementations/`
3. **单元测试**：`pytest tests/ -v --cov=implementations`
