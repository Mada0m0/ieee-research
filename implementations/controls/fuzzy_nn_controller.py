...
class FuzzyNNController:
    ...
    Use Gaussian Membership Functions and
    The TSK (Takagi-Sugeno-Kang) rule consequent performs nonlinear mapping and is suitable for compensating hysteresis errors.
    An online adaptive learning law based on gradient descent is used to adjust the parameters.
    """

    def __init__(self, n_rules: int = 7, n_inputs: int = 2):
        """
        Initialize the Fuzzy-NN controller.

        Args:
            n_rules (int): Number of fuzzy rules.
            n_inputs (int): Dimensions of input variables.
        """
        ...
        # Cache forward propagation variables for back propagation
        self._cache = {}
    ...
    def forward(self, x: np.ndarray) -> np.ndarray:
        """
        Forward propagation computes the output of a fuzzy neural network.

        Args:
            x (np.ndarray): Input vector with shape (n_inputs,).

        Returns:
            np.ndarray: Controls the output, an array of shape (1,) (containing a single scalar value).
        """
        ...
        # 3. Normalized firing strength layer (Normalized firing strength)
        sum_w = np.sum(w)
        if sum_w == 0:
        ...
        # Save the forward propagation results for online adaptive back propagation
        self._cache = {
            'x': x,
            'x_ext': x_ext,
            'mu': mu,
    ...
    def train(self, X_train: np.ndarray, y_train: np.ndarray, epochs: int = 100, lr: float = 0.01):
        """
        Offline training model (simplified version of batch mode, implemented by calling adapt_online multiple times).

        Args:
            X_train (np.ndarray): training input, shape (n_samples, n_inputs).
            y_train (np.ndarray): training output target, shape (n_samples,).
    ...
    def adapt_online(self, x: np.ndarray, y_true: np.ndarray, lr: float = 0.001):
        """
        Online adaptive update step (gradient descent).

        Adjust the center, width and TSK weight of the Gaussian function in real time based on the system error.
        You need to execute forward first to fill the cache.

        Args:
            x (np.ndarray): current input vector.
            y_true (np.ndarray): Current true output target or reference value.
            lr (float): learning rate.
        """
        # Calculate forward once and get the latest cache (the previous one can be reused in the control loop, but recalculated for safety reasons)
        y_pred = self.forward(x)
        error = y_pred[0] - y_true[0]
        ...
        if sum_w == 0:
            return # Avoid division by zero errors causing gradient explosion
        ...
        for r in range(self.n_rules):
            ...
            # Simplify derivative calculation:
    ...
    def get_fuzzy_rules(self) -> List[str]:
        """
        Extract and format the current interpretable fuzzy rules.

        Returns:
            List[str]: String list of fuzzy rules.
        """
        rules = []
        for r in range(self.n_rules):
            antecedents = []
    ...
    def plot_membership_functions(self, save_path: Optional[str] = None):
        """
        Visualize the current Gaussian membership function.
